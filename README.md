...
Current public release: **v0.5.7** (2026-04-15).
...
- hotspot set includes tiny conv and conv->attn bottleneck shapes:
  - conv: `16x16`, `24x24`, `32x32` (`n=1,c=3->16,k=3,s=1,p=1`)
  - conv->attn: `(seq, d) = (48,48), (96,48), (192,48), (192,8)`
...
- `runtime_top_op_path`: runtime-direct hotspot (`op|selected_device|direct_or_fallback`)
- `api_lightning_top_op_path`: lc.api(lightning) hotspot from timeline grouping
- `runtime_dispatch_per_iter`, `api_lightning_dispatch_per_iter`: dispatch events per iter
- `runtime_fallback_per_iter`, `api_lightning_fallback_per_iter`: fallback dispatches per iter
...
- `dispatch_reduction_pct`: host-dispatch reduction rate of graph vs eager (`> 0` means graph has fewer host dispatches)
- `host_dispatch_reduction_rate_pct` (JSON summary): fixed per-run reduction coverage metric (`reduced_cases / ok_cases * 100`)
...
# 35. Roadmap
Roadmap baseline is now aligned to **v0.5.7** and tracked in detail in [ROADMAP.md](ROADMAP.md).
...
57. [completed] v0.4.8 stabilization patch: tune-cache v2 header/version management + docs-pages metadata sync + local artifact cleanup policy hardening.
58. [completed] v0.5.0 phase E contract freeze: `engine_federation_contract.json` source-of-truth lock + CI sync checker hard-gate.
59. [completed] v0.5.1 CoreML engine adapter alpha: Python runner adapter exposure + deterministic fallback telemetry/reason codes.
60. [completed] v0.5.2 MLX bridge alpha: runtime-optional MLX bridge path + reason-coded fallback/copy telemetry.
61. [completed] v0.5.3 engine federation policy v2: unified schema (`lightning/torch/tf/coreml/mlx/auto`) and queryable route-policy surfaces.
62. [completed] v0.5.4 import/export compatibility matrix automation: README/docs matrix generated from canonical JSON with CI sync checks.
63. [completed] v0.5.5 interop perf gate v2: bridge-level reason coverage + boundary decomposition + overhead budget hard-gates.
64. [completed] v0.5.6 phase E exit audit: release-candidate evidence bundle with matrix/docs/artifact-hash lock.
65. [completed] v0.5.7 release/test matrix automation: `test_matrix_contract.json` + auto-generated README/docs matrix + CI/docs check wiring.
66. [completed] v0.5.8 Phase F contract docs/gates: `phase_f_framework_contract.json` + generator + workflow sync checker.
67. [completed] v0.5.9 federation policy v3 surfaces: `engine_federation_policy_v3_schema` + resolver API on `lc.api`.
68. [completed] v0.5.10 bridge telemetry v3: DLPack-first torch/tf bridge with `boundary_bridge_path_*` + `zero_copy_effective`.
69. [completed] v0.5.11 CoreML roundtrip beta path: checkpoint reload parity + optional runtime benchmark artifact flow.
70. [completed] v0.5.12~v0.5.15 preflight hardening: modular Python facade scaffold, C API/ABI freeze helpers/tests, release workflow Phase F audit, and release-metadata sync expansion.
...
## 35.4 Release/Test Matrix

Source of truth:
- `docs/test_matrix_contract.json`
- `docs/tested_environments.json`
- generated docs: `docs/test_matrix.md`
...
## 35.5 Phase F RC Contracts

Source of truth:
- `docs/phase_f_framework_contract.json`
- generated docs: `docs/phase_f_contracts.md`

<!-- AUTO-PHASE-F-CONTRACT:BEGIN -->

### Phase F RC Contract (Auto-generated)

- Contract version: `phase_f_v0.6.0_rc0_lock`
- As-of date: `2026-04-15`
- Source of truth: `docs/phase_f_framework_contract.json`
- CI sync checker: `python scripts/check_phase_f_contract_sync.py`

#### API/ABI Freeze Baseline
...
#### CI Constants (Phase F RC Audit)

| Key | Value |
| --- | --- |
| require_deprecation_policy_section | True |
| require_phase_e_exit_bundle | True |
| require_phase_f_docs_sync | True |
| require_semver_sync | True |
| require_test_matrix_sync | True |

<!-- AUTO-PHASE-F-CONTRACT:END -->
