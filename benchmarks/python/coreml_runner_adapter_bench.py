#!/usr/bin/env python3
"""CoreML runner adapter alpha benchmark.

Evaluates deterministic fallback coverage and boundary-overhead budget fields for
CoreML adapter paths. This alpha benchmark is designed to pass even when no
CoreML model file is provided by validating reason-code coverage.
"""
...
def main() -> None:
    ...
    p.add_argument("--max-boundary-overhead-ms", type=float, default=8.0)
    ...
    p.add_argument("--csv", type=str, default="coreml_runner_adapter.csv")
    p.add_argument("--json", type=str, default="coreml_runner_adapter.json")
    p.add_argument("--md", type=str, default="coreml_runner_adapter.md")
    args = p.parse_args()
    ...
    runtime_cases = [
        {"name": "runtime_unavailable", "enable_coreml_runtime": False, "model_path": ""},
        {"name": "missing_model", "enable_coreml_runtime": True, "model_path": ""},
    ]
    if str(args.coreml_model_path).strip():
        runtime_cases.append(
            {
                "name": "model_provided",
                "enable_coreml_runtime": True,
                "model_path": str(args.coreml_model_path).strip(),
            }
        )
    ...
    for case in runtime_cases:
        ...
        for input_kind in input_kinds:
            ...
            for mode in modes:
                adapter = lc_api.create_coreml_model_runner_adapter(
                    runner,
                    mode=mode,
                    device=args.device,
                    route_policy={"conv": "auto", "attention": "auto", "graph": "auto"},
                    coreml_model_path=str(case["model_path"]),
                    allow_missing_runtime=True,
                    enable_coreml_runtime=bool(case["enable_coreml_runtime"]),
    ...
    payload = {
        ...
        "suite": "coreml_runner_adapter",
        "artifact_schema_version": "coreml_runner_adapter_artifact_v1",
        ...
        "adapter_schema": lc_api.coreml_runner_adapter_schema() if hasattr(lc_api, "coreml_runner_adapter_schema") else {},
        "rows": rows,
    }
